...
print("<font color=\"#00fa9a\"><b>HTTF Ezreal:</b> </font><font color=\"#FFFFFF\">Server status: </font><font color=\"#ff0000\"><b>"..Server.."</b></font>")
...
function Variables()
  ...
  QTargetRange = Q.range+200
  WTargetRange = W.range+200
  RTargetRange = R.range+200
  
  QMinionRange = Q.range+200
  QJunglemobRange = Q.range+200
...
function EzrealMenu()
  ...
  Menu:addSubMenu("Draw Settings", "Draw")
    ...
    if Smite ~= nil then
      Menu.Draw:addParam("Blank3", "", SCRIPT_PARAM_INFO, "")
    Menu.Draw:addParam("S", "Draw Smite range", SCRIPT_PARAM_ONOFF, true)
    end
      Menu.Draw:addParam("Blank4", "", SCRIPT_PARAM_INFO, "")
...
function Combo()
  ...
  local ComboQ2 = Menu.Combo.Q2
    ...
    if Q.ready and ComboQ and ComboQ2 <= ManaPercent() then
      ...
      if ValidTarget(QTarget, Q.range+200) then
        CastQ(QTarget)
      end
      ...
      if QHitChance == 0 then
        ...
        for i, enemy in ipairs(EnemyHeroes) do
          ...
          if ValidTarget(enemy, Q.range+200) then
            CastQ(enemy)
          end
  ...
  if WTarget ~= nil then
    ...
    if W.ready and ComboW and ComboW2 <= ManaPercent() and ValidTarget(WTarget, W.range+200) then
      CastW(WTarget)
    end
  ...
  if RTarget ~= nil then
    ...
    if R.ready and ComboR and .8*RTargetDmg >= RTarget.health and ValidTarget(RTarget, R.range+200) then
      CastR(RTarget)
    end
...
function Farm()
  ...
  if Q.ready and FarmQ and FarmQ2 <= ManaPercent() then
    ...
    for i, minion in pairs(EnemyMinions.objects) do
      ...
      if QMinionDmg >= minion.health and ValidTarget(minion, Q.range+200) then
        CastQ(minion)
      end
    ...
    for i, minion in pairs(EnemyMinions.objects) do
      ...
      if QMinionDmg+2.5*AAMinionDmg <= minion.health and ValidTarget(minion, Q.range+200) then
        CastQ(minion)
      end
...
function JFarm()
  ...
  if Q.ready and JFarmQ and JFarmQ2 <= ManaPercent() then
    ...
    for i, junglemob in pairs(JungleMobs.objects) do
      ...
      if LargeJunglemob ~= nil and GetDistance(LargeJunglemob, mousePos) <= Q.range and ValidTarget(LargeJunglemob, Q.range+200) then
    ...
    for i, junglemob in pairs(JungleMobs.objects) do
      ...
      if ValidTarget(junglemob, Q.range+200) then
        CastQ(junglemob)
      end
...
function Harass()
  ...
  if WTarget ~= nil then
    ...
    if W.ready and HarassW and HarassW2 <= ManaPercent() and ValidTarget(WTarget, W.range+200) then
      CastW(WTarget)
    end
  ...
  if S.ready and JStealS then
    ...
    for i, junglemob in pairs(JungleMobs.objects) do
      ...
      for j = 1, #FocusJungleNames do
        ...
        if junglemob.name == FocusJungleNames[j] and SJunglemobDmg >= junglemob.health and ValidTarget(junglemob, S.range+200) then
          CastS(junglemob)
          return
        end
  ...
  if Q.ready and JStealQ then
    ...
    for i, junglemob in pairs(JungleMobs.objects) do
      ...
      for j = 1, #FocusJungleNames do
        ...
        if junglemob.name == FocusJungleNames[j] and QJunglemobDmg >= junglemob.health and ValidTarget(junglemob, Q.range+200) then
          CastQ(junglemob)
        end
  ...
  if S.ready then
    ...
    for i, junglemob in pairs(JungleMobs.objects) do
      ...
      for j = 1, #FocusJungleNames do
        ...
        if junglemob.name == "SRU_Baron12.1.1" or junglemob.name == "SRU_Dragon6.1.1" and SJunglemobDmg >= junglemob.health and ValidTarget(junglemob, S.range+200) then
          CastS(junglemob)
          return
        end
  ...
  if Q.ready then
    ...
    for i, junglemob in pairs(JungleMobs.objects) do
      ...
      for j = 1, #FocusJungleNames do
        ...
        if (junglemob.name == "SRU_Baron12.1.1" or junglemob.name == "SRU_Dragon6.1.1") and QJunglemobDmg >= junglemob.health and ValidTarget(junglemob, Q.range+200) then
          CastQ(junglemob)
        end
...
function KillSteal()
  ...
  for i, enemy in ipairs(EnemyHeroes) do
    ...
    if Q.ready and KillStealQ and QTargetDmg >= enemy.health and ValidTarget(enemy, Q.range+200) then
      CastQ(enemy)
    end
    
    if W.ready and KillStealW and WTargetDmg >= enemy.health and ValidTarget(enemy, W.range+200) then
      CastW(enemy)
    end
...
function GetDmg(spell, enemy)
  ...
  elseif spell == "R" then
    ...
    if R.ready then
      APDmg = 150*R.level+150+AddDmg+.9*AP
    end
...
function CastQ(unit)
  ...
  if QPos ~= nil and QHitChance >= 1 then
...
function CastW(unit)
  ...
  if WPos ~= nil and WHitChance >= 1 then
...
function CastR(unit)
  ...
  if RPos ~= nil and RHitChance >= 1 then
...
function OnDraw()
  ...
  if Menu.Draw.PP.W and WPos ~= nil then
    ...
    DrawCircle(WPos.x, WPos.y, WPos.z, W.width/2, color)
    
    if Menu.Draw.PP.Line then
      DrawLine3D(myHero.x, myHero.y, myHero.z, WPos.x, WPos.y, WPos.z, 2, color)
    end
